...
from data_util.model_util_scannet import rotate_aligned_boxes
...
class ScannetDetectionDataset(Dataset):
    ...
    def __init__(self, split_set='train', num_points=20000,
        use_color=False, use_height=False, augment=False, data_path = os.path.join(BASE_DIR, 'scannet_train_detection_data')):
        ...
        if split_set=='all':            
            ...
            split_filenames = os.path.join(ROOT_DIR, 'data_util/meta_data',
    ...
    def __getitem__(self, idx):
        ...
        mesh_vertices, _ = pc_util.random_sampling(mesh_vertices, 50000, return_choices=True)
        ...
        # print("sem_cls_label",[DC.class2type[i] for i in ret_dict['sem_cls_label']])
        # print("sem_cls_label",ret_dict['sem_cls_label'])
...
class CA1MDetectionDataset(Dataset):
       
    def __init__(self, data_root='/media/lyq/temp/dataset/CA-1M-slam/'):
        
        self.data_root=data_root
        self.scan_names = sorted(os.listdir(self.data_root))
    ...
    def __getitem__(self, idx):
        ...
        Returns a dict with following keys:
        """
        cur_seq = os.path.join(self.data_root, self.scan_names[idx])


        gt_npy = os.path.join(cur_seq,'after_filter_boxes.npy')
        corners_array = np.load(gt_npy)


        ret_dict = {}
        ret_dict['gt_corners'] = corners_array
        ret_dict['scan_idx'] = self.scan_names[idx]
...
def viz_obb(pc, label, mask, angle_classes, angle_residuals,
    ...
    print("detected {} GT oriented boxes".format(len(oriented_boxes)))
