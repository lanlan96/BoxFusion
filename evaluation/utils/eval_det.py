...
from box_util import box3d_iou, box3d_iou_v2
def get_iou_obb(bb1,bb2):
    iou3d, iou2d = box3d_iou(bb1,bb2)
    return iou3d

def get_iou_obb_v2(bb1,bb2):
    iou3d, iou2d = box3d_iou_v2(bb1,bb2)
    return iou3d
        ...
        if BBGT.size > 0:
            ...
            for j in range(BBGT.shape[0]):
                ...
                if iou > ovmax:
                    ovmax = iou
                    jmax = j
            # print("iou debug",'detected:',d, "gt:",jmax,"iou:", ovmax)
        #print d, ovmax
        if ovmax > ovthresh:
            if not R['det'][jmax]:
                tp[d] = 1.
...
def eval_det_multiprocessing(pred_all, gt_all, ovthresh=0.25, use_07_metric=False, get_iou_func=get_iou):
    ...
    # ret_values = eval_det_cls_wrapper([pred[0], gt[0], ovthresh, use_07_metric, get_iou_func]) 
